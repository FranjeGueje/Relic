...
export async function installOrUpdateTool(tool: Tool) {
  ...
  if (tool.name === 'dxvk-macOS' && asset.name.includes('-builtin')) {
    ...
    // env variable, check the associated PR for more details
...
function getDxvkUrl(): string {
  ...
  if (any_gpu_supports_version([1, 1, 0])) {
    logInfo(
      'The GPU(s) in this system only support Vulkan 1.1/1.2, falling back to DXVK 1.10.3 for Wine',
      LogPrefix.ToolInstaller
    )
...
function getVkd3dUrl(): string {
  ...
  if (any_gpu_supports_version([1, 3, 0])) {
    ...
    return 'https://api.github.com/repos/HansKristian-Work/vkd3d-proton/releases/latest'
  }
  if (any_gpu_supports_version([1, 1, 0])) {
    logInfo(
      'The GPU(s) in this system only support Vulkan 1.1/1.2, falling back to VKD3D 2.6 for Wine',
      LogPrefix.ToolInstaller
    )
