...
- **Nile ya no sincroniza dos veces al arrancar**: `init()` cargaba la biblioteca por red además del refresh que dispara el frontend. Ahora solo carga de disco, como Legendary. De paso, `refreshNile()` pasa a esperarse: sin `await`, la biblioteca se reconstruía leyendo los ficheros que el sync aún estaba escribiendo.
- **Zoom verificaba el login 3 veces por arranque**: `isLoggedIn()` hace una petición de red, y `getUserDetails()` la llamaba y luego repetía el mismo endpoint. Ahora es una sola petición compartida, con dedup y ventana de 60 s. Además, un fallo de red ya no borra el token del usuario: solo un 401/403 de Zoom cierra la sesión. 12 tests nuevos.
